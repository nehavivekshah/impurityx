...
class FrontendController extends Controller
{
    ...
    public function productDetailsPost(Request $request, $product_slog)
    {
        try {
            ...
            // Determine Financial Year (e.g., 2526 for 2025-2026)
            $currentMonth = date('n');
            if ($currentMonth >= 4) {
                $fyPart1 = date('y');
                $fyPart2 = date('y', strtotime('+1 year'));
            } else {
                $fyPart1 = date('y', strtotime('-1 year'));
                $fyPart2 = date('y');
            }
            $financialYear = $fyPart1 . $fyPart2;

            // Get next sequence number for this financial year
            $maxSeq = Orders::where('financial_year', $financialYear)->max('fy_sequence');
            $nextSeq = $maxSeq ? ($maxSeq + 1) : 1;

            $order = new Orders();
            $order->financial_year = $financialYear;
            $order->fy_sequence = $nextSeq;
        ...
        if ($page == 'my-orders' || $page == 'delivery-acceptance') {
            ...
            if ($user->role == 5) {
                $headers = array_merge($headers, [
                    ...
                    "My Offered Price",
                    "My Offered Days",
                    "My Delivery Deadline",
                    "Accepted Storage Condition",
                    ...
                    "GST Amt",
                    "Gross Order Value",
                    "Order Intiation Dt By Seller",
                    "Order Task Completion Dt By Seller",
                    "Delivery Initiation  Dt By Seller",
                    "Delivery Completion Dt By Seller",
                    "Delivery Acceptance Dt By Buyer",
    ...
    public function sellerMyAccount($page, Request $request)
    {
        ...
        if ($page == 'dashboard') {
            ...
            return view(
                'frontend/seller/seller-dashboard',
