...
class OrderController extends Controller
{
    ...
    public function updateAuctionEnd(Request $request){
        ...
        if ($order->status == 'active') {
            ...
            foreach ($sellers as $seller) {
                ...
                $details = [
                    ...
                    'order_id'          => date('y', strtotime($product->created_at)) . (date('y', strtotime($product->created_at)) + 1) .'-'. str_pad($order->id, 4, '0', STR_PAD_LEFT),
                    'sku'               => $product->sku ?? 'N/A',
        ...
        if ($order->status == 'awarded' && $getBidding) {
            ...
            if ($buyer && $buyer->email && $seller) {
                $details = [
                    'buyer_name'            => $buyer->first_name . ' ' . $buyer->last_name,
                    'order_id'              => date('y', strtotime($order->created_at)) . (date('y', strtotime($order->created_at)) + 1) .'-'. str_pad($order->id, 4, '0', STR_PAD_LEFT),
    ...
    public function exportOrderMaster(Request $request)
    {
        ...
        // CSV headers
        $headers = [
            'Sr No', 'Order ID', 'Product Name', 'SKU', 'CAS No', 'Quantity', 'UOM', 'Specific Requirements',
            'Delivery Date', 'Delivery Location', 'Auction End', 'Status', 'Seller Status',
            'Attachments', 'Buyer Name', 'Buyer Email', 'Buyer Mobile', 'Company',
            'City', 'State', 'Country', 'Created At'
        ];
    ...
    public function exportSellerBiddings(Request $request)
    {
        ...
        foreach ($biddings as $k => $b) {
            if($b->bidding_status == 'awarded'){ $status = ucfirst($b->order_status); }else{ $status = "--"; }
            $row = [
                $k + 1,
                $b->bidding_id,
                date('y', strtotime($b->created_at)) . (date('y', strtotime($b->created_at)) + 1) . '-' . str_pad($b->order_id, 4, '0', STR_PAD_LEFT),
                $b->product_name ?? 'N/A',
                $b->product_sku ?? 'N/A',
                $b->product_cas ?? 'N/A',
                $b->order_quantity ?? 0,
