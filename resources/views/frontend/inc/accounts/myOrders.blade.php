            ...
            <table id="example" class="table table-striped border rounded" style="width:100%;font-size: 14px;">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="min-width:70px!important;"><strong>{{ $financialYear . '-' . str_pad($order->id, 4, '0', STR_PAD_LEFT) }}</strong></td>
                        ...
                        <td class="text-center">
                            ...
                            @if(!empty($attachments))
                                @foreach($attachments as $file)
                                    <a href="{{ asset('public/' . $file) }}" class="btn btn-primary btn-sm" target="_blank" style="font-size:12px;font-weight:600;">View</a>
                                @endforeach
                            @else
                                N/A
                            @endif
                        </td>
                        <td class="text-center" style="min-width:70px!important;">@if(!empty($order->l1_rate) && ($order->l1_rate > 0)) Rs. {{ $order->l1_rate }} @else -- @endif</td>
                        <td class="text-center" style="min-width:120px!important;">@if($order->status == 'awarded'){{ \Carbon\Carbon::parse($order->awarded_date)->format('d M, Y') }}@endif</td>
                        <td class="text-center" style="min-width:120px!important;">@if(!empty($order->auction_end)) {{ \Carbon\Carbon::parse($order->auction_end)->format('d M, Y H:i') }} @else -- @endif</td>
                        <td class="text-center">
                        ...
                        <td class="text-center" style="min-width:120px!important;">{{ \Carbon\Carbon::parse($order->created_at)->format('d M, Y') }}</td>
                        <td class="text-center sticky-column" style="min-width:90px!important;">
                            <a href="javascript:void(0)" id="{{ $order->id }}" class="btn btn-sm btn-info view" title="View Order Details" data-page="orders">
                                <i class="bx bx-show"></i>
                            </a>
                            
                            @if(empty($order->reject_order))
                            <a href="javascript:void(0)" id="{{ $order->id }}" class="btn btn-sm btn-warning view" title="View Bidding Details" data-page="bids">
