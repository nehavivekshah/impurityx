        ...
        <div class="col-md-12 px-3 bg-white">
            <table id="example" class="table table-striped border rounded" style="width:100%;font-size: 14px;">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $key + 1 }}</td>
                        <td><strong>{{ date('y', strtotime($order->order_date)) . (date('y', strtotime($order->order_date)) + 1) . '-' . str_pad($order->order_id, 4, '0', STR_PAD_LEFT) }}</strong></td>
                        ...
                        <td>{{ $order->expdly_dt ? \Carbon\Carbon::parse($order->expdly_dt)->format('d M, Y') : '--' }}</td>
                        <td>{{ $order->dly_city ?? '--' }}</td>
                        <td>{{ $order->bid_end_time ? \Carbon\Carbon::parse($order->bid_end_time)->format('d M, Y H:i') : '--' }}</td>
                        <td>
