            ...
            @foreach($posts as $post)
                <div class="col-xl-4 col-lg-6 col-md-6 col-sm-10">
                    <div class="single-blog-style1 wow fadeInDown" data-wow-duration="1.5s" data-wow-delay=".2s">
                        <div class="blog-img">
                            <a href="/blog/{{ $post->categoryRelation->slog }}/{{ $post->slog }}" class="blog-date">
                                <i class="bi bi-calendar-check"></i>
                                {{ $post->created_at ? $post->created_at->format('d M, Y') : '' }}
                            </a>
                            <a href="/blog/{{ $post->categoryRelation->slog }}/{{ $post->slog }}">
                                <img alt="{{ $post->title ?? '' }}" src="{{ asset('public/assets/frontend/img/posts/' . ($post->imgs ?? 'default.jpg')) }}">
                            </a>
                        </div>
                        <div class="blog-content">
                            ...
                            <div class="blog-meta">
                                <div class="comment">
                                    <img alt="{{ $post->category ?? 0 }}" src="{{ asset('public/assets/frontend/images/icons/tags.svg') }}">
                                    <a href="javascript:void(0)" class="comment">
                                        {{ ucfirst($post->category ?? 0) }}
                                    </a>
                                </div>
                                <div class="author">
                                    <img alt="{{ $post->author ?? '' }}" src="{{ asset('public/assets/frontend/images/blog/user.png') }}">
