...
@section('title','Reset Password - Impurity X')
@section('headlink')
    ...
    <div class="scrum-board-container">
        <div class="flex justify-content-center">
            <div class="col-md-4 rounded bg-white mb-3">
                <h1 class="h3 box-title pb-2 text-center">
                    Reset Password
                </h1><hr>
                <form action="{{ route('managePassword') }}" method="POST" class="row card-body pb-3 px-3">
                    @csrf
                    <div class="form-group col-md-12">
                        <label class="small">Password*</label><br>
                        <div class="input-group">
                            <img src="{{ asset('/assets/backend/icons/lock.svg'); }}" class="input-icon" />
                            <input type="password" name="password" class="form-control" id="pass" placeholder="Password" required />
                        </div>
                    </div>
                    <div class="form-group col-md-12">
                        <label class="small">Re-enter Password*</label><br>
                        <div class="input-group">
                            <img src="{{ asset('/assets/backend/icons/lock.svg'); }}" class="input-icon" />
                            <input type="password" name="repassword" class="form-control" id="repass" placeholder="Re-enter Password" required />
                        </div>
                    </div>
...
@section('footlink')
<!-- Scripts -->
<script src="{{ asset('/assets/bacnkend/js/script.js') }}"></script>
<script>
    $(document).ready(function(){
        $('#repass').keyup(function(){
            if($('#pass').val() == $(this).val()){
                ...
                $('#sbtn').removeAttr("disabled", "disabled");
