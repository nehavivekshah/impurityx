                            ...
                            @foreach($biddings as $key => $bid)
                                @php
                                    if (!empty($bid->financial_year) && !empty($bid->fy_sequence)) {
                                        $financialYear = $bid->financial_year;
                                        $orderIdDisplay = $financialYear . '-' . str_pad($bid->fy_sequence, 3, '0', STR_PAD_LEFT);
                                    } else {
                                        ...
                                        $orderIdDisplay = $financialYear . '-' . str_pad($bid->order_id, 4, '0', STR_PAD_LEFT);
                                    }
                                @endphp
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>
                                        <strong>{{ $orderIdDisplay }}</strong><br>
                                    ...
                                    <td>
                                        ...
                                        @if(Auth::user()->role == '1' || (in_array('4', $access) && in_array('4', $actions)))
                                            <a href="javascript:void(0)" class="notify alert-danger delete" id="{{ $bid->id }}"
                                                data-page="delBidStatus" title="Delete" @if($bid->status == 'awarded')
                                                style="pointer-events:none;cursor:default;opacity:.5;" @endif><i
