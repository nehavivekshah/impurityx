<style>
    ...
    .is-readonly { background-color:#f1f1f1; cursor:not-allowed; }
            ...
            <table id="example" class="table table-striped border rounded" style="width:100%;font-size: 14px;">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><strong><?php echo e(date('y', strtotime($order->created_at)) . (date('y', strtotime($order->created_at)) + 1) . '-' . str_pad($order->id, 4, '0', STR_PAD_LEFT)); ?></strong><br>
                            <?php echo e($order->first_name . ' ' . $order->last_name); ?><br>
                            <?php if($order->status == 'awarded'): ?><small><?php echo e($order->email); ?></small><?php endif; ?>
                        </td>
                        <td>
                            <?php echo $order->sku ?? ''; ?><br>
                            <?php echo e($order->name ?? ''); ?><br>
                            <small><strong>CAS No.:</strong> <?php echo e($order->cas_no ?? ''); ?></small>
                        </td>
                        <td>
                        ...
                        <td class="text-center"><?php if(!empty($order->auction_end)): ?> <?php echo e(\Carbon\Carbon::parse($order->auction_end)->format('d M, Y H:i')); ?> <?php else: ?> -- <?php endif; ?></td>
                        <td class="text-center">
                        ...
                        <td class="text-center" width="90px">
                            <a href="javascript:void(0)" id="<?php echo e($order->id); ?>" class="btn btn-sm btn-info view" title="View Details" data-page="sellerawardedorders">
                                <i class="bx bx-show"></i>
                            </a>
                            <?php if($order->seller_status != 'accepted'): ?>
                            <a href="javascript:void(0)" class="btn btn-sm btn-dark edit-auction"
                               ...
                               data-invoiceno="<?php echo e($order->invoice_no); ?>"
                               data-couriername="<?php echo e($order->courier_name); ?>"
                               data-trackingid="<?php echo e($order->tracking_id); ?>"
                               data-courierdate="<?php echo e($order->courier_date); ?>"
                               data-invoicedate="<?php echo e($order->invoice_date); ?>"
                               data-date="<?php echo e(\Carbon\Carbon::parse($order->invoice_date)->format('Y-m-d\TH:i')); ?>">
                               <i class="bx bx-edit"></i>
                            </a>
                            <?php endif; ?>
                            <a href="/seller/dbaction?selectorId=<?php echo e($order->id); ?>&pagename=sellerawardedordersexport" class="btn btn-sm btn-primary" title="Download Order's Details">
...
<section class="editAuctionModal">
    <div class="modal-md">
        ...
        <div class="editContent p-3">
            <form id="editOrderForm" method="POST" action="/seller/update-order-seller-status" style="min-height: 290px;">
                <?php echo csrf_field(); ?>
                ...
                <div class="mb-3">
                    ...
                    <input type="date" class="form-control" name="invoice_date" id="invoice_date" max="<?php echo e(date('Y-m-d')); ?>">
                </div>
...
<?php $__env->stopSection(); ?><?php /**PATH /home/u321815713/domains/impurityx.com/public_html/resources/views//frontend/seller/inc/accounts/myOrders.blade.php ENDPATH**/ ?>